from apps.recordmanagement.models import RecordTemplate, RecordEncryptionNew
from apps.static.encryption import RSAEncryption
from apps.api.models import Rlc, Permission, UsersRlcKeys
...
class TestUserKeys(TestCase):
    def setUp(self):
        ...
        self.record_1 = data.create_record(template=self.template, users=[self.user_1['user'], self.user_2['user']])
        self.record_2 = data.create_record(template=self.template, users=[self.user_1['user'], self.user_2['user']])
        self.client = Client()
    ...
    def test_record_key_check(self):
        self.get_user_record_key(self.record_1, self.user_1).test(self.user_1['private_key'])
        assert self.get_user_rlc_keys(self.user_1).correct
    ...
    def test_list_keys(self):
        ...
        assert RecordEncryptionNew.objects.filter(user=self.user_1['user']).count() + 1 == len(response_data)
    ...
    def test_delete_key(self):
        ...
        response = c.delete('/api/keys/{}/'.format(key_id))
        assert response.status_code == 400
        encryption = RecordEncryptionNew(record=self.record_1['record'],
