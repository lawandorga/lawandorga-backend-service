...
class UserBase:
    def setUp(self):
        ...
        self.user1, self.rlc_user1 = self.create_user('dummy@law-orga.de', 'Dummy', settings.DUMMY_USER_PASSWORD)
        self.user2, self.rlc_user2 = self.create_user('tester@law-orga.de', 'Tester', 'pass1234')
...
class UserUnitTests(UserBase, TestCase):
    ...
    def test_generate_keys_for_user_works_with_unlocking_user_having_the_keys(self):
        ...
        private_key = UserProfile.objects.get(pk=self.user2.pk).get_private_key('pass12345')
        success = self.user1.generate_keys_for_user(self.private_key1, UserProfile.objects.get(pk=self.user2.pk))
