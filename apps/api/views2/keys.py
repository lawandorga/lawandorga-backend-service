...
DELETE_KEY_ERROR_NOT_ENOUGH = 'User {} tried to delete a record key that only one other person or nobody has.'
...
def delete_key(data: KeyDelete, user: UserProfile):
    ...
    if key is None:
        return ServiceResult(DELETE_KEY_ERROR_NOT_FOUND, error='The key you want to delete does not exist.')
    if key.record.encryptions.count() <= 2:
        return ServiceResult(DELETE_KEY_ERROR_NOT_ENOUGH, error='Not enough people have access to this record. '
                                                                'There need to be at least 2 people who must '
                                                                'have access. You can not delete this key.')
    if key.correct:
        return ServiceResult(DELETE_KEY_ERROR_WORKS, error='You can not delete a key that works.')
    key.delete()
