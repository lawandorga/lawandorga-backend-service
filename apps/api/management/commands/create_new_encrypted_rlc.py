...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        admin_keys = UserEncryptionKeys(
            user=admin_user, private_key=admin_private, public_key=admin_public
        )
        admin_keys.save()
        ...
        rlc_keys = RlcEncryptionKeys(
            rlc=rlc, encrypted_private_key=encrypted_private, public_key=rlc_public
        )
        rlc_keys.save()
        for_user_encrypted_rlc_key = RSAEncryption.encrypt(rlc_aes, admin_public)
        admin_rlc_keys = UsersRlcKeys(
            user=admin_user, rlc=rlc, encrypted_key=for_user_encrypted_rlc_key
        )
        admin_rlc_keys.save()
