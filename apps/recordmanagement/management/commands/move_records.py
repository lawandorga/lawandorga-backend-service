    ...
    with transaction.atomic():
        ...
        if old_record.record_token:
            RecordStandardEntry.objects.create(
                record=record, field=field, value=old_record.record_token
            )
        #
        ...
        if old_record.first_contact_date:
            RecordStandardEntry.objects.create(
                record=record, field=field, value=old_record.first_contact_date
            )
        #
        field = RecordStandardField.objects.get(
        ...
        if old_record.last_contact_date:
            RecordStandardEntry.objects.create(
                record=record, field=field, value=old_record.last_contact_date
            )
        #
        field = RecordStandardField.objects.get(
            template=template, name="First consultation"
        )
        if old_record.first_consultation:
            RecordStandardEntry.objects.create(
                record=record, field=field, value=old_record.first_consultation
            )
        #
        field = RecordStandardField.objects.get(template=template, name="Official Note")
        if old_record.official_note:
            RecordStandardEntry.objects.create(
                record=record, field=field, value=old_record.official_note
            )
        #
        ...
        if old_record.note:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.note
            )
        #
        ...
        if old_record.consultant_team:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.consultant_team
            )
        #
        field = RecordEncryptedStandardField.objects.get(
        ...
        if old_record.lawyer:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.lawyer
            )
        #
        field = RecordEncryptedStandardField.objects.get(
        ...
        if old_record.related_persons:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.related_persons
            )
        #
        field = RecordEncryptedStandardField.objects.get(
        ...
        if old_record.contact:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.contact
            )
        #
        field = RecordEncryptedStandardField.objects.get(
        ...
        if old_record.bamf_token:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.bamf_token
            )
        #
        field = RecordEncryptedStandardField.objects.get(
        ...
        if old_record.first_correspondence:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.first_correspondence
            )
        #
        field = RecordEncryptedStandardField.objects.get(
        ...
        if old_record.circumstances:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.circumstances
            )
        #
        field = RecordEncryptedStandardField.objects.get(
        ...
        if old_record.next_steps:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.next_steps
            )
        #
        field = RecordEncryptedStandardField.objects.get(
        ...
        if old_record.status_described:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.status_described
            )
        #
        field = RecordEncryptedStandardField.objects.get(
        ...
        if old_record.additional_facts:
            RecordEncryptedStandardEntry.objects.create(
                record=record, field=field, value=old_record.additional_facts
            )
        # client entries
        ...
        if old_record.client.birthday:
            RecordStandardEntry.objects.create(
                record=record, field=field, value=old_record.client.birthday
            )
        #
...
def test_record(record):
    ...
    if old_record.first_contact_date:
        ...
        assert str(old_record.first_contact_date) == entry.value, "{} != {}".format(
            old_record.first_contact_date, entry.value
        )
    #
    field = RecordStandardField.objects.get(template=template, name="Last contact date")
    if old_record.last_contact_date:
        entry = RecordStandardEntry.objects.get(record=new_record, field=field)
        assert str(old_record.last_contact_date) == entry.value, "{} != {}".format(
            old_record.last_contact_date, entry.value
        )
    #
    field = RecordStandardField.objects.get(
        template=template, name="First consultation"
    ...
    if old_record.first_consultation:
        entry = RecordStandardEntry.objects.get(record=new_record, field=field)
        assert str(old_record.first_consultation) == entry.value, "{} != {}".format(
            old_record.first_consultation, entry.value
        )
    #
    field = RecordStandardField.objects.get(template=template, name="Official Note")
    if old_record.official_note:
        entry = RecordStandardEntry.objects.get(record=new_record, field=field)
        assert str(old_record.official_note) == entry.value, "{} != {}".format(
            old_record.official_note, entry.value
        )
    #
    field = RecordUsersField.objects.get(template=template, name="Consultants")
    if old_record.working_on_record:
        ...
        assert set(
            list(old_record.working_on_record.values_list("pk", flat=True))
        ) == set(list(entry.value.values_list("pk", flat=True))), "{} != {}".format(
            list(old_record.working_on_record.values_list("pk", flat=True)),
    ...
    if old_record.tags.values_list("name", flat=True):
        ...
        assert (
            list(old_record.tags.values_list("name", flat=True)) == entry.value
        ), "{} != {}".format(
            list(old_record.tags.values_list("name", flat=True)), entry.value
        )
    #
    ...
    if old_record.get_state_display().capitalize():
        ...
        assert (
            str(old_record.get_state_display().capitalize()) == entry.value
        ), "{} != {}".format(old_record.get_state_display, entry.value)
    #
    field = RecordEncryptedStandardField.objects.get(template=template, name="Note")
    if old_record.note:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert old_record.note == entry.value, "{} != {}".format(
            old_record.note, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(
        template=template, name="Consultant Team"
    ...
    if old_record.consultant_team:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert old_record.consultant_team == entry.value, "{} != {}".format(
            old_record.consultant_team, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(template=template, name="Lawyer")
    if old_record.lawyer:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert old_record.lawyer == entry.value, "{} != {}".format(
            old_record.lawyer, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(
        template=template, name="Related Persons"
    ...
    if old_record.related_persons:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert old_record.related_persons == entry.value, "{} != {}".format(
            old_record.related_persons, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(template=template, name="Contact")
    if old_record.contact:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert (old_record.contact) == entry.value, "{} != {}".format(
            old_record.contact, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(
        template=template, name="BAMF Token"
    ...
    if old_record.bamf_token:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert (old_record.bamf_token) == entry.value, "{} != {}".format(
            old_record.bamf_token, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(
        template=template, name="First Correspondence"
    ...
    if old_record.first_correspondence:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert (old_record.first_correspondence) == entry.value, "{} != {}".format(
            old_record.first_correspondence, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(
        template=template, name="Circumstances"
    ...
    if old_record.circumstances:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert (old_record.circumstances) == entry.value, "{} != {}".format(
            old_record.circumstances, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(
        template=template, name="Next Steps"
    ...
    if old_record.next_steps:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert (old_record.next_steps) == entry.value, "{} != {}".format(
            old_record.next_steps, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(
        template=template, name="Status described"
    ...
    if old_record.status_described:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert (old_record.status_described) == entry.value, "{} != {}".format(
            old_record.status_described, entry.value
        )
    #
    field = RecordEncryptedStandardField.objects.get(
        template=template, name="Additional facts"
    ...
    if old_record.additional_facts:
        entry = RecordEncryptedStandardEntry.objects.get(record=new_record, field=field)
        assert (old_record.additional_facts) == entry.value, "{} != {}".format(
            old_record.additional_facts, entry.value
        )
    # client entries
    assert new_record.old_client == old_record.client, "{} != {}".format(
        record.old_client.pk, old_record.client.pk
    )
    #
    # field = RecordEncryptedStandardField.objects.get(template=template, name='Client name')
    # if old_record.client.name:
    ...
    if old_record.client.birthday:
        entry = RecordStandardEntry.objects.get(record=new_record, field=field)
        assert str(old_record.client.birthday) == entry.value, "{} != {}".format(
            old_record.client.birthday, entry.value
        )
    # #
    field = RecordSelectField.objects.get(template=template, name="Origin Country")
    if old_record.client.origin_country:
        ...
        assert (
            str(old_record.client.origin_country.name) == entry.value
