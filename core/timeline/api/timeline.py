...
@router.post("")
def command__create_timeline_event(rlc_user: RlcUser, data: schemas.InputTimelineEventCreate):
    create_timeline_event(rlc_user, data.text, data.folder_uuid)


@router.post("delete/")
def command__create_timeline_event(rlc_user: RlcUser, data: schemas.InputTimelineEventDelete):
