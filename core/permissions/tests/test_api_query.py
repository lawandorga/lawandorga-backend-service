...
def test_query_permissions_filters_by_user_id():
    ...
    user_profile = UserProfile.objects.create(
        email="testuser@law-orga.de", name="Test User"
    )
    ...
    response_filtered = client.get(
        f"/api/permissions/query/permissions/?user_id={org_user.pk}"
    )
    ...
    assert (
        perm_to_assign.pk not in result_ids
...
def test_query_permissions_filters_by_group_id():
    ...
    response_filtered = client.get(
        f"/api/permissions/query/permissions/?group_id={group.pk}"
    )
    assert response_filtered.status_code == 200
    filtered_result = response_filtered.json()
