...
class RecordsView(models.Model):
    ...
    def create(cls, name: str, user: RlcUser, columns=list[str], pk=0) -> "RecordsView":
        record = RecordsView(name=name, columns=columns, org=user.org)
        if pk:
            record.pk = pk
        return record

    name = models.CharField(max_length=200)
    user = models.ForeignKey(RlcUser, related_name="records_views", on_delete=models.CASCADE)
    ...
    def __str__(self):
        return "recordsView: {}; user: {};".format(
