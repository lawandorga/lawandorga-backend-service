...
class FolderModelTests(TransactionTestCase):
    ...
    def test_size_updating_with_files(self):
        ...
        file3 = File(name='file3', size=200, creator=self.fixtures['users'][0]['user'], folder=bottom_folder)
        file3.save()
    ...
    def test_get_folder_from_path(self):
        ...
        self.assertEqual(bottom_folder,
                         Folder.get_folder_from_path('top folder/middle folder/bottom folder/', self.fixtures['rlc']))
        self.assertEqual(bottom_folder,
                         Folder.get_folder_from_path('top folder/middle folder/bottom folder', self.fixtures['rlc']))
        self.assertEqual(bottom_folder, Folder.get_folder_from_path('top folder/middle folder/bottom folder/asd.pdf',
                                                                    self.fixtures['rlc']))
    ...
    def test_user_has_permission(self):
        ...
        self.assertFalse(created_folders['ressorts'].user_can_see_folder(self.fixtures['users'][0]['user']))

        perm1 = PermissionForFolder(folder=created_folders['ressorts'], permission=p_read,
                                    group_has_permission=self.fixtures['groups'][0])
        perm1.save()

        self.assertTrue(created_folders['ressorts'].user_has_permission_read(self.fixtures['users'][0]['user']))
        self.assertTrue(created_folders['public'].user_has_permission_read(self.fixtures['users'][0]['user']))
        self.assertTrue(created_folders['post_date'].user_has_permission_read(self.fixtures['users'][0]['user']))
        self.assertFalse(created_folders['ressorts'].user_has_permission_read(self.fixtures['users'][2]['user']))
        self.assertFalse(created_folders['ressorts'].user_can_see_folder(self.fixtures['users'][2]['user']))

        perm2 = PermissionForFolder(folder=created_folders['instagram'], permission=p_read,
                                    group_has_permission=self.fixtures['groups'][1])
        perm2.save()

        self.assertTrue(created_folders['ressorts'].user_can_see_folder(self.fixtures['users'][2]['user']))
        self.assertTrue(created_folders['public'].user_can_see_folder(self.fixtures['users'][2]['user']))
        self.assertFalse(created_folders['ressorts'].user_has_permission_read(self.fixtures['users'][2]['user']))
...
# def test_download_folder(self):
    ...
    def test_download_real(self):
        ...
        file2.save()
        middle_folder = Folder(name='beirat', creator=self.fixtures['users'][0]['user'], rlc=self.fixtures['rlc'],
                               parent=top_folder)
        middle_folder.save()
    ...
    def create_many_test_folders(self):
        created_folders = {}
        folder_ressorts = Folder(name='Ressorts', creator=self.fixtures['users'][0]['user'], rlc=self.fixtures['rlc'])
        folder_ressorts.save()
        created_folders.update({'ressorts': folder_ressorts})

        folder_public = Folder(name='Oeffentlichkeitsarbeit', creator=self.fixtures['users'][0]['user'],
                               rlc=self.fixtures['rlc'], parent=folder_ressorts)
        folder_public.save()
        created_folders.update({'public': folder_public})

        folder_social = Folder(name='Social media', creator=self.fixtures['users'][0]['user'], rlc=self.fixtures['rlc'],
                               parent=folder_public)
        folder_social.save()
        created_folders.update({'social': folder_social})

        folder_instagram = Folder(name='Instagram', creator=self.fixtures['users'][0]['user'], rlc=self.fixtures['rlc'],
                                  parent=folder_social)
        folder_instagram.save()
        created_folders.update({'instagram': folder_instagram})

        folder_posts = Folder(name='Posts', creator=self.fixtures['users'][0]['user'], rlc=self.fixtures['rlc'],
                              parent=folder_instagram)
        folder_posts.save()
        created_folders.update({'posts': folder_posts})

        folder_post_date = Folder(name='24.12.2014', creator=self.fixtures['users'][0]['user'],
                                  rlc=self.fixtures['rlc'], parent=folder_posts)
        folder_post_date.save()
        created_folders.update({'post_date': folder_post_date})

        folder_stories = Folder(name='Stories', creator=self.fixtures['users'][0]['user'], rlc=self.fixtures['rlc'],
                                parent=folder_instagram)
        folder_stories.save()
        created_folders.update({'stories': folder_stories})

        folder_facebook = Folder(name='Facebook', creator=self.fixtures['users'][0]['user'], rlc=self.fixtures['rlc'],
                                 parent=folder_social)
        folder_facebook.save()
        created_folders.update({'facebook': folder_facebook})

        folder_fundraising = Folder(name='Fundraising', creator=self.fixtures['users'][0]['user'],
                                    rlc=self.fixtures['rlc'], parent=folder_ressorts)
        folder_fundraising.save()
        created_folders.update({'fundraising': folder_fundraising})

        folder_template = Folder(name='Vorlagen', creator=self.fixtures['users'][0]['user'], rlc=self.fixtures['rlc'])
    ...
    def test_get_all_parents(self):
        ...
        middle_folder = Folder(name='middle folder', creator=self.fixtures['users'][0]['user'],
                               rlc=self.fixtures['rlc'], parent=top_folder)
        middle_folder.save()
        middle_folder_2 = Folder(name='middle folder 2', creator=self.fixtures['users'][0]['user'],
                                 rlc=self.fixtures['rlc'], parent=top_folder)
    ...
    def test_get_all_children(self):
        ...
        self.assertCountEqual([middle_folder_2, middle_folder, bottom_folder], top_folder.get_all_children())
