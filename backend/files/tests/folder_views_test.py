...
from backend.static.permissions import PERMISSION_ACCESS_TO_FILES_RLC, PERMISSION_READ_ALL_FOLDERS_RLC
...
class FolderViewsTest(TransactionTestCase):
    ...
    def test_get_folder_information(self):
        access_permission: Permission = Permission.objects.get(name=PERMISSION_ACCESS_TO_FILES_RLC)
        has_access_permission: HasPermission = HasPermission(permission=access_permission,
                                                             group_has_permission=self.fixtures['groups'][0],
                                                             permission_for_rlc=self.fixtures['rlc'])
        has_access_permission.save()
        read_permission: Permission = Permission.objects.get(name=PERMISSION_READ_ALL_FOLDERS_RLC)
        has_read_permission: HasPermission = HasPermission(permission=read_permission,
                                                           group_has_permission=self.fixtures['groups'][0],
                                                           permission_for_rlc=self.fixtures['rlc'])
    ...
    def test_get_download_folder(self):
        ...
        has_permission = HasPermission(permission=access, group_has_permission=self.fixtures['groups'][0],
                                       permission_for_rlc=self.fixtures['rlc'])
        has_permission.save()
