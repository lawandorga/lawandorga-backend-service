...
class MissingRlcKeysTests(TransactionTestCase):
    ...
    def test_full_flow(self):
        ...
        response_from_login = client.post('/api/login/', {'username': users_email, 'password': 'qwe123'})
        self.assertEqual(200, response_from_login.status_code)
        self.assertEqual(2, record_models.RecordEncryption.objects.filter(user=api_models.UserProfile.objects.get(email=users_email)).count())
