    ...
    def generate_encryption_keys_for_all_users():
        ...
        for user in users:
            ...
            user_keys = UserEncryptionKeys(
                user=user, private_key=private, public_key=public
            )
            user_keys.save()
        ...
        for rlc in rlcs:
            ...
            for user in users_from_rlc:
                ...
                for_user_encrypted_rlc_aes_key = RSAEncryption.encrypt(
                    aes_key, users_public
                )
                user_rlc_keys = UsersRlcKeys(
                    user=user, rlc=rlc, encrypted_key=for_user_encrypted_rlc_aes_key
                )
                user_rlc_keys.save()
...
class Migrators:
    ...
    def encrypt_record(record):
        ...
        potential_e_client = EncryptedClient.objects.filter(
            from_rlc=client.from_rlc,
        ...
        if not potential_e_client:
            e_client = EncryptedClient(
                from_rlc=client.from_rlc,
                created_on=client.created_on,
                last_edited=client.last_edited,
                birthday=client.birthday,
            ...
            e_client.phone_number = AESEncryption.encrypt(
                client.phone_number, client_key
            )
            e_client.save()
        ...
        e_record = EncryptedRecord(
            ...
            first_consultation=record.first_consultation,
        ...
        AESEncryption.encrypt_field(
            record, e_record, "first_correspondence", e_record_key
        )
        ...
        for message in messages:
            e_message = EncryptedRecordMessage(
                sender=message.sender, record=e_record, created_on=message.created_on
            )
        ...
        for permission in permissions:
            e_permission = EncryptedRecordPermission(
                request_from=permission.request_from,
        ...
        for deletion_request in deletion_requests:
            e_deletion_request = EncryptedRecordDeletionRequest(
                request_from=deletion_request.request_from,
    ...
    def decrypt_record_document(record_document, e_record, e_record_key):
        e_record_document = EncryptedRecordDocument(
            record=e_record,
            creator=record_document.creator,
            name=record_document.name,
            created_on=record_document.created_on,
        ...
        try:
            ...
            EncryptedStorage.download_file_from_s3(
                record_document.get_file_key(), local_path_to_file
            )
