...
class EncryptedRecordTests(TransactionTestCase):
    ...
    def test_create_and_get_encrypted_record(self):
        ...
        response: Response = client.post('/api/records/e_record/', new_record_object, format='json',
        ...
        response: Response = client.get('/api/records/e_record/' + str(response.data['id']) + '/', format='json',
                                        **{'HTTP_PRIVATE_KEY': private_key})
    ...
    def test_create_encrypted_record_cant_consult(self):
        ...
        response: Response = client.post('/api/records/e_record/', new_record, format='json',
                                         **{'HTTP_PRIVATE_KEY': private_key})
    ...
    def test_get_encrypted_record(self):
        ...
        response: Response = client.get('/api/records/e_record/' + str(record.id) + '/',
                                        **{'HTTP_PRIVATE_KEY': private_key})
        self.assertEqual(200, response.status_code)
    ...
    def test_patch_encrypted_record(self):
        ...
        self.assertEqual(record_note, AESEncryption.decrypt(new_record_from_db.note, record_key))

        response: Response = client.patch('/api/records/e_record/' + str(new_record_from_db.id) + '/', {
            "record": {
                "note": "new note",
        ...
        self.assertEqual('new note', AESEncryption.decrypt(new_record_from_db.note, record_key))

        response_record_tagged_ids = [tag['id'] for tag in response.data['record']['tagged']]
    ...
    def test_patch_encrypted_record_wrong_id(self):
        ...
        response: Response = client.patch('/api/records/e_record/' + str(123123) + '/', {
            "record": {
                "note": "new note"
            },
            "client": {
        ...
        record: record_models.EncryptedRecord = self.record_fixtures['records'][0]['record']
        response: Response = client.patch('/api/records/e_record/' + str(record.id) + '/', {
            "record": {
                "tagged": [40001],
            }
        }, format='json', **{'HTTP_PRIVATE_KEY': private_key})
