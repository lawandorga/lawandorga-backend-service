...
class EncryptedRecordDeletionRequestViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        if not request.user.has_permission(permissions.PERMISSION_PROCESS_RECORD_DELETION_REQUESTS,
                                           for_rlc=request.user.rlc) and not request.user.is_superuser:
            raise CustomError(error_codes.ERROR__API__PERMISSION__INSUFFICIENT)
        if request.user.is_superuser:
            queryset = models.EncryptedRecordDeletionRequest.objects.all()
        else:
            queryset = models.EncryptedRecordDeletionRequest.objects.filter(request_from__rlc=request.user.rlc)
        return Response(serializers.EncryptedRecordDeletionRequestSerializer(queryset, many=True).data)
    ...
    def create(self, request):
        ...
        if not record.user_has_permission(request.user):
            raise CustomError(error_codes.ERROR__API__PERMISSION__INSUFFICIENT)
        if models.EncryptedRecordDeletionRequest.objects.filter(record=record, state='re').count() >= 1:
            raise CustomError(error_codes.ERROR__RECORD__RECORD_DELETION__ALREADY_REQUESTED)
        deletion_request = models.EncryptedRecordDeletionRequest(request_from=request.user, record=record,
        ...
        return Response(serializers.EncryptedRecordDeletionRequestSerializer(deletion_request).data)


class EncryptedRecordDeletionProcessViewSet(APIView):
    def post(self, request):
        user = request.user
        if not user.has_permission(permissions.PERMISSION_PROCESS_RECORD_DELETION_REQUESTS, for_rlc=user.rlc):
            raise CustomError(error_codes.ERROR__API__PERMISSION__INSUFFICIENT)

        if 'request_id' not in request.data:
            raise CustomError(error_codes.ERROR__API__ID_NOT_PROVIDED)
        try:
            record_deletion_request = models.EncryptedRecordDeletionRequest.objects.get(pk=request.data['request_id'])
        except:
        ...
        record_deletion_request.processed_on = datetime.utcnow().replace(tzinfo=pytz.utc)
        if action == 'accept':
            ...
            record_deletion_request.save()
        response_request = models.EncryptedRecordDeletionRequest.objects.get(pk=record_deletion_request.id)
        return Response(serializers.EncryptedRecordDeletionRequestSerializer(response_request).data)
